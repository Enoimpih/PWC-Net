...
sys.path.append("PyTorch/models")
...
def predict_flow(in_planes):  # in the bottom level of the pyramid, there will no leakyReLu but just one conv layer
    return nn.Conv2d(in_planes, 2, kernel_size=3, stride=1, padding=1, bias=True)


def deconv(in_planes, out_planes, kernel_size=4, stride=2,
           padding=1):  # components, can take it as interpolation/upsampling
...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        # each pyramid level has fixed but different paras of warping layers, flow estimator and context network
        # each pyramid level contains 5 conv layers
        nd = (2 * md + 1) ** 2  # 81
        ...
        # context network: contains 7 conv layers, based on dilated conv(something that can combine information from nearby frames)`
        self.dc_conv1 = conv(od + dd[4], 128, kernel_size=3, stride=1, padding=1, dilation=1)  # bottom level
    ...
    def forward(self, x):
        ...
        flow2 = flow2 + self.dc_conv7(self.dc_conv6(self.dc_conv5(x)))  # output
...
class PWCDCNet_old(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dc_conv4(self.dc_conv3(self.dc_conv2(self.dc_conv1(x))))
        flow2 = flow2 + self.dc_conv7(self.dc_conv6(self.dc_conv5(x)))  # refined flow

        if self.training:  # when training, return flows from each layer
            return flow2, flow3, flow4, flow5, flow6
        else:
            return flow2  # when testing, only return the final flow


def pwc_dc_net(path=None):  # create a model with paras
    model = PWCDCNet()
    if path is not None:
        data = torch.load(path)  # load model
        if 'state_dict' in data.keys():
            model.load_state_dict(data['state_dict'])  # load paras
        else:
